...
describe('MetricsQueryService.getPopulationDiversity（平台人群多样性 surface）', () => {
  ...
  beforeEach(() => {
    ...
    /* Task 5：ctor 收 resolver（单库=SingleDbResolver，scatter-gather 下 allShardDbs()=[db]，等价现状）。
     * getPopulationDiversity 返回 ShardAggregate<PersonalityDiversityResult>，度量在 .data。 */
    metrics = new MetricsQueryService(new SingleDbResolver(db));
  });
  ...
  it('空平台（无任何租户）→ population=0 diversityScore=0', () => {
    ...
    assert.equal(d.data.diversityScore, 0);
  });
  ...
  it('单租户 → population=1 diversityScore=0（无成对可比）', () => {
    ...
    assert.equal(d.data.diversityScore, 0, '单租户无成对距离');
  });
  ...
  it('多租户（出生扰动开启）→ population=N diversityScore>0', () => {
    ...
    const d = metrics.getPopulationDiversity();
    assert.equal(d.data.count, 4, '应统计全部 4 个租户的决策风格');
    assert.ok(d.data.diversityScore > 0, `多租户群体应有多样性，实际 ${d.data.diversityScore}`);
    /* perDimensionSpread/Mean 应是合法 [0,1] 数值（非 NaN）。 */
    for (const v of Object.values(d.data.perDimensionSpread)) {
  ...
  it('跨租户聚合：不被任一租户隔离（读到所有租户）', () => {
    ...
    const d = metrics.getPopulationDiversity();
    assert.equal(d.data.count, 2, '跨租户应同时读到 iso-a 与 iso-b');
    assert.ok(d.data.diversityScore > 0, 'riskAppetite 0.1 vs 0.9 应产生多样性');
  });
  ...
  it('畸形 style_json 行被跳过，不污染度量也不抛错', () => {
    ...
    const d = metrics.getPopulationDiversity();
    /* 畸形行被跳过，仅合法行计入（count=1）；不抛错、不降级。 */
    assert.equal(d.degraded, false);
    assert.equal(d.data.count, 1, '畸形 style_json 行应被跳过');
  });
  ...
  it('TTL 缓存：TTL 内复用上次结果，过期后重算（避免高频 scrape O(n²) 阻塞）', () => {
    ...
    assert.equal(cached.data.count, 2, 'TTL 内应复用缓存，不反映新增租户');

    /* 过 TTL 后：重算，反映全部 3 个租户。 */
    const recomputed = metrics.getPopulationDiversity(1_040_000); /* +40s > 30s */
    assert.equal(recomputed.data.count, 3, 'TTL 过期后应重算');
  });
});
