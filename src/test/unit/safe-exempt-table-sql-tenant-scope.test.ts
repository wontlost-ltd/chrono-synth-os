...
const ALLOWED_GLOBAL_SQL: readonly AllowedGlobalSql[] = [
  ...
  { table: 'tasks', globalKind: 'worker_global_semantics',
    fingerprint: "update tasks set status = 'running', claimed_by = ?, claimed_at = ${dbnowms(db)}, updated_at = ${dbnowms(db)} where id = ? and status = 'pending'",
    reason: 'worker 全局 dequeue 后按 id claim（时刻由 DB 盖戳，见 #395）' },
  { table: 'tasks', globalKind: 'worker_global_semantics',
  ...
  { table: 'tasks', globalKind: 'worker_global_semantics',
    fingerprint: 'update tasks set status = \'pending\', claimed_by = null, claimed_at = null, available_at = ${dbnowms(db)}, updated_at = ${dbnowms(db)}, retry_count = retry_count + 1 where ${stalecondition} and retry_count < max_retries',
    reason: 'worker reaper 回收僵死 running task（时刻由 DB 盖戳，见 #395）' },
  { table: 'tasks', globalKind: 'worker_global_semantics',
    fingerprint: "update tasks set status = 'failed', error = ?, claimed_by = null, claimed_at = null, updated_at = ${dbnowms(db)} where ${stalecondition} and retry_count >= max_retries",
    reason: 'worker reaper 终结超重试的僵死 task（时刻由 DB 盖戳，见 #395）' },
