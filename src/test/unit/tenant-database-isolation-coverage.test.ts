...
describe('TenantDatabase isolation ratchet: tenant tables must be isolated or registered', () => {
  ...
  it('回归锚点：本轮 GDPR 纳管的代表性表已进入隔离集', () => {
    for (const t of ['response_templates', 'persona_rules', 'llm_provider_credentials', 'tenant_llm_settings', 'user_oauth_tokens', 'conversation_messages', 'legal_holds', 'api_keys']) {
