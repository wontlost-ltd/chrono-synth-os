...
describe('数字员工组织全生命周期 E2E（M1→D→A/B/C→M2/M3/M5/M7）', () => {
  ...
  /**
   * ADR-0057 L2：标记某 persona 已学会某能力（落 passed 学习请求）。
   * 现实里发布岗 persona 执行发布任务前会先学会 'publishing'（学习闭环）；本 E2E 测的是 D2/D3 执行路径，
   * 故在执行前直接把所需能力标为已学，让缺口门放行（缺口门本身在 L2 专测覆盖）。
   */
  const markLearned = (personaId: string, capability: string): void => {
    const lr = new LearningRequestStore(os.getDatabase(), ctx.tenantId);
    lr.insert({
      id: `seed-${personaId}-${capability}`, orgId: 'org-1', personaId, capability,
      isUnknown: false, evidence: 'E2E seed', priority: 'low', triggeredByTaskId: null,
      status: 'passed', createdAt: clock.now(), updatedAt: clock.now(),
    });
  };
  ...
  it('② D2+D3：HTTP 请求审批(high→pending) → 人类 approve → 触发真实执行(接管线，写回 submitted/blocked)', async () => {
    ...
    /* ADR-0057 L2：发布岗 persona 先学会 'publishing'（发布任务所需能力），否则缺口门会挡执行（learning_required）。 */
    markLearned('p-p', 'publishing');
