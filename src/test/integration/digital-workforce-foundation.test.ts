...
describe('数字员工组织奠基（M1：分解-委派-执行-汇报-聚合，零-LLM）', () => {
  ...
  it('完整因果链：主管运行内容目标 → 分解4任务 → 委派 → 执行 → 汇报 → 聚合', () => {
    ...
    /* M2 审计：目标记录了产生它的 playbook 版本。 */
    assert.equal(result.playbookVersion, 1, 'M2 参考版本 v1');
    assert.equal(store.getGoal('org-1', result.goalId)!.playbookVersion, 1, 'goal 落库 playbook 版本');
