...
export class MetaRegulationLayer {
  ...
  detectConflicts(personas: readonly PersonaVersion[]): void {
    ...
    for (const c of valueConflicts) {
      this.bus.emit('meta:conflict-detected', { conflict: c, tenantId: this.tenantId });
    ...
    if (resourceConflict) {
      this.bus.emit('meta:conflict-detected', { conflict: resourceConflict, tenantId: this.tenantId });
  ...
  proposeIntegration(result: SimulationResult): IntegrationProposal {
    ...
    this.bus.emit('meta:integration-proposed', { proposal, tenantId: this.tenantId });
  ...
  decideIntegration(
    ...
    this.bus.emit('meta:integration-decided', { proposalId: proposal.id, accepted, tenantId: this.tenantId });
  ...
  allocateResources(personas: readonly PersonaVersion[], strategy?: AllocationStrategy): ResourceAllocation[] {
    ...
    this.bus.emit('meta:resources-allocated', { allocations, tenantId: this.tenantId });
  ...
  resolveConflict(conflictId: string, resolution: string): boolean {
    ...
    if (ok) {
      this.bus.emit('meta:conflict-resolved', { conflictId, resolution, tenantId: this.tenantId });
