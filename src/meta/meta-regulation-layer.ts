...
export class MetaRegulationLayer {
  ...
  constructor(
    ...
    this.integrator = new IntegrationEngine(clock, integrationConfig);
  ...
  /** 决定并执行集成 */
  decideIntegration(proposal: IntegrationProposal, fitnessScore: number, coreLayer: CoreRhythmLayer): boolean {
    ...
    if (accepted) {
      this.integrator.apply(proposal, coreLayer);
