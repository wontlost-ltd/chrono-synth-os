...
export function registerSsoRoutes(app: FastifyInstance, db: IDatabase, config: AppConfig): void {
  ...
  app.get('/api/v1/auth/sso/callback', ssoRateLimit, async (request, reply) => {
    ...
    const pendingState = await stateStore.getAndDelete(state);
    if (!pendingState) {
    ...
    try {
      ...
      const user = ssoUserService.findOrCreateForSso(userInfo.email);
      const issued = await generateTokenPair(app, db, config, user.userId, user.tenantId, user.role);

      const redirectUrl = new URL(pendingState.redirectUri, baseUrl);
