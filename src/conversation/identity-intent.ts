 ...
 *     把「你叫服务员过来」「我叫你别乱动」「你叫得真大声」挡在外面；
 *   - 之后必须紧接句尾（只允许语气词/标点）。 */
const BARE_NAME_CHARS = "[^\\s，。,.!！?？、的了吧呀啊呢嘛吗什么谁啥来去过下别乱动一声我他她它们个把要让给帮叫喊得真服务员外卖起床过来]{1,6}";
...
const NAME_DEFINE_PATTERNS: readonly RegExp[] = [
  ...
  new RegExp(`(?:以后|从今(?:以后|往后)?|从现在起)?你(?:就|以后)?叫\\s*(${BARE_NAME_CHARS})${NAME_TAIL}`),    // 你叫X（句尾）
];
