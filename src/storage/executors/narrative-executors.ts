...
export function registerNarrativeExecutors(): void {
  registerQuery<string, NarrativeGetParams>(NARRATIVE_QUERY_GET, (db: IDatabase, params) => {
    const row = db.prepare<NarrativeRow>(
      'SELECT content FROM narrative WHERE tenant_id = ? AND persona_id = ?',
  ...
  registerCommand<NarrativeSetParams>(NARRATIVE_CMD_SET, (db: IDatabase, p) => {
    db.prepare<void>(
      `INSERT INTO narrative (tenant_id, persona_id, content, updated_at) VALUES (?, ?, ?, ?)
       ON CONFLICT(tenant_id, persona_id) DO UPDATE SET content = excluded.content, updated_at = excluded.updated_at`,
    ).run(p.tenantId, p.personaId, p.content, p.updatedAt);
