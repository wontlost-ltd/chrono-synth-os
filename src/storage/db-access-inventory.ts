...
export const DB_ACCESS_INVENTORY: readonly DbAccessPoint[] = [
  ...
  { id: 'src/server/routes/companion/learn-github.ts#registerCompanionLearnGithubRoutes', file: 'src/server/routes/companion/learn-github.ts', category: 'longlived-root-capture', note: '注册期 :82 sharedDb=db??os.getDatabase() 绑 QuotaManager.fromResolver（同 companion/chat 款；handler 内另有 tenantOS.getDatabase() 走已路由 tenantOS，属 tenant-isolated 语义，但文件级 ratchet 以最重的 sharedDb 捕获归类）' },
  { id: 'src/server/routes/personas.ts#registerPersonaRoutes', file: 'src/server/routes/personas.ts', category: 'longlived-root-capture', note: '注册期 PersonaCoreService.fromResolver(new SingleDbResolver(os.getDatabase())) 长寿命（仍捕获 root os.getDatabase()；双入口化后 service 持 source，待 app.ts 穿真 ShardRouter 消除）' },
  { id: 'src/server/routes/admin-templates.ts#registerAdminTemplateRoutes', file: 'src/server/routes/admin-templates.ts', category: 'longlived-root-capture', note: '注册期 tx=os.getDatabase() 绑 SingleDbResolver→PersonaCoreService/PersonaTemplateService 长寿命（同 personas 款）' },
  ...
  { id: 'src/server/routes/companion/draft-github-reply.ts#registerCompanionDraftGithubRoutes', file: 'src/server/routes/companion/draft-github-reply.ts', category: 'tenant-isolated', note: 'handler 内 tenantOS.getDatabase()（:130/:136/:198 建 GithubAppCredentialStore/查 installation/GithubDraftStore），tenantOS 已经 getOS(request) 路由；无注册期 sharedDb 捕获、无长寿命子服务绑定——纯按租户路由后取 db，Phase 0 TenantOSFactory 换源后自动生效' },
  // —— explicit-per-request（GitHub webhook：系统入站，handler 内一次性取 db，无长寿命绑定） ——
  { id: 'src/server/routes/github-webhook.ts#registerGithubWebhookRoutes', file: 'src/server/routes/github-webhook.ts', category: 'explicit-per-request', note: ':151 os.getDatabase() 作 installation→tenant 反查（读平台级 github_installations 映射，靠 UNIQUE(host,installation) 定位未知租户，故用基座 DB 不带 tenant 过滤）；:160/:180 反查出 tenantId 后经 tenantOSFor(tenantId) 路由再 tenantOS.getDatabase() 验签/存草稿。全部在 request handler 内当场取，未捕获成字段/未绑子服务生命周期' },
