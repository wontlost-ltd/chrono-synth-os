...
| **Edge-P2（已落地）** | Non-Node kernel runtime proof：端侧确定性 host adapter（零 node:*）驱动真实 kernel value-service + golden replay + 零-node ratchet | 新（ADR-0001 兑现） |
| **Edge-P3（已落地）** | 端侧持久化往返（serialize/restore + EdgePersistence）+ SyncOutbox（device+单调 seq）+ 冲突解决三分法（fact 合并去重 / projection 重建 / **identity 冲突 pending 绝不 last-write-wins**） | Phase 3 部分 |
| **Edge-P4（已落地）** | 离线成长队列（GrowthJobQueue 持久化）+ TeacherJobRunner（联网批量消费，teacher 输出走蒸馏门，**失败隔离不阻断 runtime 离线自治**）；teacher 绝不进 runtime sense | Phase 2 **推迟** |
| **Edge-P5（已落地）** | perception_media_refs 表（v086，原始媒体绝不进库只存引用元数据）+ MediaRefStore + retention worker（按 delete_after 清理+触发对象存储 erase，失败隔离）+ GDPR（导出脱敏不含 object_key + 擦除）+ ObjectStorageEraser 可注入（真 S3/R2 部署接入） | Phase 3 余项 |
| **Edge-P6（规格完成）** | 固件人格子集规格（ADR-0053）：哪些 kernel 能力进 MCU 首版（value 快照/规则子集/环境状态机/节律/ring buffer/outbox），哪些不进（memory graph/蒸馏门/teacher/身份核写入/原始媒体）；真固件是独立跨工具链工程 | 新 |
