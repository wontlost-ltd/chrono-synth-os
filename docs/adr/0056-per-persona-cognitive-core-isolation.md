...
> 这是**目标态**。当前实现进度（截至 K5）：人格特征三件套 narrative / decision_style / cognitive_model 已按
> `(tenant, persona)` 隔离（K2 executor 已扩）；values / memories（含 edges）/ survival 的 `persona_id` 列已加
> 但 executor 尚未扩维，运行时仍 tenant 键——**待 K5b 完成宽表 executor 扩维**后这两类才达成 per-persona 隔离。
> 详见下方分片路线 K5 / K5b 与「K5 实际隔离边界诚实声明」。
...
> 实现进度：截至 K5，硬不变量在**人格特征三件套**（narrative / decision_style / cognitive_model）上完整成立；
> values / memories / edges 的 `persona_id` 列已加但 executor 未扩、运行时仍 tenant 键——**K5b 后**此三类才达成硬不变量。
...
K5 M3/M5 persona-aware：蒸馏编译经 resolver 落该 persona 的人格特征内核（narrative/decision_style/
  ...
  │  └─ K5b（后续）：宽表 ValueStore/CognitiveMemoryGraph 的 (tenant, persona) executor 扩维——
  │     value_shift/memory_edge 当前仍 tenant 共享，待 K5b 才 per-persona 隔离
  │
  ▼
...
| K5b | 宽表 executor 扩维 | K5 | `ValueStore`/`CognitiveMemoryGraph` executor 扩 `(tenant, persona)`；value_shift/memory_edge 编译达成 per-persona 隔离 | p-alice 的 value/memory 编译不影响 p-bob/default；K5 的边界用例反转为隔离断言 | 中高 | 高 | K0 覆盖 |
...
**正面**（完整态；当前进度见 D0.2 实现进度注 + 分片路线 K5/K5b）：
- 「不同的数字人格」真正成立——同组织多 worker 各有独立认知内核、独立决策风格、独立成长轨迹。
  （截至 K5：三件套维度已成立；value/memory 维度待 K5b。）
- M3 自成长按 persona 局部学习，不污染他人（三件套维度已成立）；M5 自适应按 persona 调度，认知层真多主体。
