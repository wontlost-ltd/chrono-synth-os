...
K4 org bootstrap 真实接入：为每 worker 创建独立 persona core + 套 archetype + 初始化最低可运行 core state
  │
  ▼
K5 M3/M5 persona-aware：autorun 按 persona 调度（org=预算/治理边界，persona=认知执行边界）；
   蒸馏写 persona-local（org-shared 需显式层）
  │
  ▼
...
| K5 | M3/M5 persona-aware | K4 | M5 按 persona 调度（org=预算边界/persona=认知边界，确定性排序）；M3 蒸馏写 persona-local | 多 worker 各用自己 core；蒸馏只影响目标 persona；tenant budget 汇总；单 persona tenant 旧行为不变 | 高 | 很高 | K0 覆盖（如 M3/M5 有独立 ADR 则追 amendment）|
...
- M3 自成长按 persona 局部学习，不污染他人；M5 自适应按 persona 调度，认知层真多主体。
