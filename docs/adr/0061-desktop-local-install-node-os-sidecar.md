...
| **S4** | **桌面构建 CI**：新 workflow 多平台 `tauri build`（macos arm64/x64、win x64、linux）+ 按目标架构下载锁版本 Node + SHASUMS 校验；签名/公证凭据经 secrets 注入（**缺省跳过出 unsigned 内测包**）；沿用既有 updater endpoint | S2 | ✅ 已实现（#259 骨架 → **#264 跨平台矩阵实跑通过**：四端全出未签名安装包 .dmg/.app（mac arm64+x64）、.msi/.exe（win）、.deb/.rpm/.AppImage（linux），各 47-155MB 含内嵌 Node+sidecar。首跑暴露并修 4 处真阻塞：core-pkg build-order、Windows npm 须 shell（Node24 .cmd EINVAL）、updater/Apple 空签名 env→拆签名/未签名两模式、build.rs 用 render-rust.js 真身非 .bin shim。⚠️ 正式签名/公证仍待外部凭据） |
| **S5** | **前端单机适配**：companion + enterprise UI 在单机 profile 下的引导（首启无需填 server URL/token）；离线降级 UX（红线 10）；企业面在单机隐藏或标注「本地单租户」 | S2、S3 | ✅ 已实现（#260） |
| **S6** | **端到端 + 冒烟**（本 PR 落地范围）：**便携 sidecar bundle 本地全链冒烟**（`e2e:desktop-local`——拷临时目录断符号链接起 bundle → 握手/红线11 → auto-provision → companion 零-LLM → per-persona 组织出生 → T7 路由接通 → SQLite 落库 → 优雅关停无孤儿）。⚠️ **真·干净机双击安装 / 卸载升级数据保全** = 生产发版前 follow-up（须实机）；跨平台安装器矩阵实跑 ✅ 已过（见 S4/#264），签名公证待外部凭据。 | S1-S5 | ✅ 已实现（本 PR，范围=便携 bundle 本地全链） |
