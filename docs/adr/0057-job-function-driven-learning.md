...
| L8b | 缺口处置：委派/降级（后续） | L8a | 缺口能力组织内有同事会 → 委派(org chart 委派链+能力匹配 CapabilityAssignmentService)换人做任务续；否则降级(保守版+标注待补)有产出 | 委派合法性确定性(直接下属/能力匹配)；降级确定性可解释不假装完成；优先级 委派>降级>挂起 | 中高 | 中 | L0 覆盖 |
