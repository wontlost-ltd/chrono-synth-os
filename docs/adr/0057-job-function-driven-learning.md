...
| L5b ✅ | 交叉审第二轮（**完成**） | L5 | kernel mergeCrossReview 纯函数(**只收紧不放松**:初审退回沿用不翻盘/初审放行+两交叉审都 endorse 才放行/任一不 endorse 退回 stage=cross_review/fail-closed) + gate crossReview opt-in 默认关(初审放行才跑第二轮省 LLM，两老师看对方草案+原始候选复核 catch 伪共识)；blind 只约束初审，第二轮看对方是设计本意。role→capability registry 仍可选留后续(ADR L5 已定 role/family 审计-only) | 只收紧不放松(退回不可能翻放行)；伪共识/理由相悖可抓；判定逻辑全 kernel 确定性(只 endorse 意见是 LLM)；opt-in 向后兼容(默认关=纯 L5) | 中 | 中 | L0 覆盖 |
...
| L8a ✅ | 学完唤醒重跑核心闭环（**完成**） | L2,L6,L7 | TaskWakeHandler 订阅 capability-learned → 经 learning_requests.triggered_by_task_id 找因该 (persona,cap) 挂起(blocked)的任务 → **重跑前确定性 GapDetector 复检**（已学=CapabilityIndex∪L2 passed）→ 无缺口唤醒(blocked→delegated，**只标 resolved 不在此执行**，真执行由正常执行门接手再过缺口门)/仍缺 **fail-closed**(保持 blocked)；org_tasks 加 resume_attempt_count + last_wake_event_id(迁移 v110/pg v112)防死循环+wake 幂等；blocked_on_capabilities **派生不存储**(GapDetector(required,已学)，消除可漂移冗余) | 任务不无限干等；**多能力缺口全清零才重跑**；复检仍缺 fail-closed(绝不静默执行/假完成)；resumeAttemptCount 上限防死循环(超限停 blocked 待 SLA/人工)；lastWakeEventId 幂等去重 EventBus 重复投递；per-persona 隔离；全程零-LLM | 中高 | 中 | L0 覆盖 |
| L8b ✅ | 缺口处置：委派/降级（**完成**） | L8a | CapabilityAssignmentService.findCapableColleague(同 org active 同事，复用 listLearnedCapabilities=CapabilityIndex∪L2 同 GapDetector 源，**学齐全部所需能力**才合格防踢皮球，稳定序确定性选首个) + TaskDispositionService(委派>降级>挂起：委派 reassignDelegatedTaskIfHeldBy CAS 锁 assignee+status=delegated；降级 opt-in 默认关，submitted+[降级]结构化标注**不假完成**)；缺口门改为登记学习后挂起前调 disposition；学习请求无论怎么处置都登记(异步补) | 委派目标学齐全部能力(接手不再撞缺口门，非踢皮球)；降级确定性可解释+显式标注不静默假完成+默认关(保守)；优先级 委派>降级>挂起；CAS 失败安全退回 suspend；per-(tenant,org) 隔离；全程零-LLM；向后兼容(未注入=L8a 行为) | 中高 | 中 | L0 覆盖 |
| L8c-wire ✅ | reconciler 生产周期触发接线（**完成**） | L8c | TaskWakeReconcilerWorker(setInterval 周期调 reconciler.reconcileTenant，同 QuotaUsageRetentionWorker 款:running 重入守卫+unref+start/stop/isHealthy+flushOnce+注入 now 可测) + reconciler 抽 reconcileTasks 共享核心(用 task.orgId，单 org reconcileOnce 与全租户 reconcileTenant 共用不漂移)+store listLearningBlockedTasksAllOrgs(本租户全 org，仍 JOIN learning_requests 收窄) + ChronoSynthOS start/stop 接线(每租户 OS 经 TenantOSFactory.os.start() 各自启动一个 worker) | 丢事件兜底**自动化**(不再只靠运维手动)；闭合 L8c「engine built steering wheel unconnected」；每租户 unref timer 轻量(10min+只扫学习 blocked+LRU 驱逐 os.close→worker.stop)；异常隔离 | 中 | 低 | L0 覆盖 |
| L8c ✅ | 唤醒可靠性：reconciler + 学习超时兜底（**完成**） | L8a | TaskWakeReconciler.reconcileOnce 反扫**因学习缺口挂起**的 blocked 任务(listLearningBlockedTasks JOIN learning_requests.triggered_by_task_id——非学习 blocked 工具失败/权限/异常**绝不**纳入)→复用 L8a wakeOneTask 同核心补唤醒(防 capability-learned 事件丢失永久挂起)；合成幂等键绑已学能力指纹(JSON.stringify，同已学状态重复反扫不烧 resumeAttemptCount 预算)；学习超时兜底标 [learning_timeout](仍 blocked 待人工/改委派)；单任务异常隔离。**按需** reconcileOnce(可测)，生产周期触发=部署接线后续。**capabilityVersion 守卫诚实跳过**(事件无此字段+index 恒 v1=想象问题，版本自增时再加) | 事件丢失不永久挂起(反扫补唤醒)；不无限挂起(超时兜底，不假完成)；幂等不烧预算；非学习 blocked 不被误唤醒/误标超时；全程零-LLM | 中 | 中 | L0 覆盖 |
