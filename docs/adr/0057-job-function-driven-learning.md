...
| L8c | 唤醒可靠性：reconciler + 学习超时兜底（后续） | L8a | 确定性 reconciler 从 CapabilityIndex 反扫 pending 任务补触发 wake(防 capability-learned 事件丢失永久挂起)+ 学习超时上限(超时升人工/改委派/标 blocked(learning_timeout)) + capabilityVersion 单调新鲜度守卫(忽略陈旧事件) | 事件丢失不永久挂起(仍 pending+超时兜底，不假完成)；不无限挂起；陈旧/乱序事件不误标 resolved | 中 | 中 | L0 覆盖 |
