...
两个**独立** LLM 老师 endpoint（复用 BYOK `ModelRouter`；两个不同 provider/key，保证独立性，禁止同一 key 充两位）。
...
- **互审模式**：两老师**各自独立**产出对「该不该学这块、是否职能相关、是否可执行落地」的 **verdict**
  ...
  **独立性强化（Codex 复审）**：初审阶段两老师**互不可见**对方草案（避免趋同/串通）；各自 verdict **固化后**
  才进入交叉审。最低独立性 = 不同 endpoint/key（红线 6），更强独立性 = 初审 blind + verdict 冻结再交叉。
...
- **ExamSpec 质量门（冻结前确定性 lint，Codex 复审：防作弊 rubric）**：冻结前必须确定性校验 rubric 健康——
  **拒绝** `.*` / 超宽 regex / 把完整答案直接塞进 alias / 零信息 keypoint / 权重异常（如单要点权重≈1 架空其他）等
  作弊路径。双老师拟题不等于 rubric 一定健康，lint 不过不得冻结。
...
- **≥95** → 候选 artifact 才**正式**经蒸馏门落主内核（D0.4 → 蒸馏门）。**<95** → 丢弃候选，回滚已是快照态。
...
6. **双老师独立**：两个老师必须不同 endpoint/key；**禁止**同一 key 充两位；两者都 approve 且都判职能相关才学。
...
```
L0 ADR 冻结（本片，纯规划）：学习模型 + 学习期/运行时边界 + 17 红线 + 分片路线
  │
  ▼
...
L4 影子内核验收：复用 K5 snapshot/coreSelfOnly，候选编译进 shadow → 确定性内核作答 → 评分 → 回滚（不污染）
  │
  ▼
L5 双老师互审门：两 LLM endpoint 独立产出 verdict + 交叉审 + 确定性职能相关性前置筛（蒸馏门之前）
  │
  ▼
...
|---|---|---|---|---|---|---|---|
| L0 | ADR 冻结 | 无 | 本 ADR（学习模型 + 边界 + 红线 + 路线） | 学习期/运行时边界、17 红线、确定性化设计冻结 | 低 | 中 | 是（本片）|
...
| L5 | 双老师互审门 | L1 | 两独立 endpoint verdict + 交叉审 + 确定性相关性筛 | 两都 approve 才学；同 key 禁；偏题退回 | 中高 | 中高 | L0 覆盖 |
...
### 可并行（很少）
- L0 后 L1（能力分类/缺口）与 L3（考试规格/评分）可局部并行（前者信号源、后者验收器，弱耦合）。
...
影子内核验收（dry-run 过同蒸馏校验 + compile lease）、知识体系沉淀方式、**17 红线**、L1-L8 分片路线。
