...
三段一条主干：所有 GitHub 学习内容收敛为**文本表征**，走文本/代码摄入契约进 `PerceptionDistiller`，事实沉淀为记忆、身份/价值提议经 `core-update-gate` 强制封顶只能 `pending`（内核绝不因读 GitHub 自动改）；所有对 GitHub 的写动作注册为 `highRisk` 工具，经 `tool-risk-deriver` 置 `toolRisk='high'` → `assessExecutionRisk` 强制人工审批，自主流永远发不出去。

```
接:  GitHubReadPort / GitHubWritePort（共享 auth-manager：App JWT→installation token 自动刷新 + SSRF allowlist + 限流）
       凭据:新建租户级 github_app_credentials 表（AES-256-GCM 加密 App 私钥/webhook secret，fail-closed）
            + github_installations（一对多 repo/org→installation_id，(github_host,installation_id) 全局唯一）
...
反馈:GitHub webhook（HMAC 签名校验 + 幂等，事件带 provider 键）→ 增量学
       ...
       →【写工具 highRisk → toolRisk=high → assessExecutionRisk 强制 requiresHuman，不可降级】
       →【人工审批门 isExecutionApprovalCleared 绑定本次执行】
       → 写工具（highRisk，唯一持有 GitHubWritePort，只经审批 executor）→ 写 → 审计留痕
```

**关键更正（对齐真实契约）**：信任层封顶只作用于**内核身份/价值**（core-update-gate），普通记忆如实沉淀（无 provenance 字段）；强制审批走 `highRisk → toolRisk='high' → requiresHuman`（**不**经 `outboundCommitment`——后者是 `ExecutionRiskSignals` 里只由 body 上调的硬信号，`highRisk` 不派生它）。
...
5. **出站唯一出口 + 读写 port 结构隔离（更正 Codex ⑧）**：所有 GitHub 网络调用只经 `GitHubReadPort`/`GitHubWritePort`（两独立接口，非同一 connector 读写方法）。**WritePort（comment/review）只在审批 executor（ToolInvocationPipeline / WorkerExecutionService）组合根注入**；学习/webhook/companion 只拿 ReadPort，编译期无 WritePort 引用 → 结构上无法绕过审批。加架构依赖测试锁死 WritePort import 边界。每次调用过 `validateOutboundUrl(rawUrl, opts)`，`opts.hostAllowlist` 锁 GitHub host。
...
### 5.1 接入段：GitHubReadPort / GitHubWritePort

**文件（新建）**：
- `src/integrations/github/github-auth-manager.ts` — App JWT 签发 + installation token 缓存/刷新。
- `src/integrations/github/github-read-port.ts` — `GitHubReadPort`（只读：repo tree / 文件 / issues / PRs / commits）。
- `src/integrations/github/github-write-port.ts` — `GitHubWritePort`（只写：issue comment / PR review；对外副作用）。
- 两 port 共享 auth-manager + ssrf-guard + fetcher；WritePort 只在审批 executor 组合根注入（约束 5、⑧）。
...
- installation token 不落库（约束 6）。**入站定位（更正 Codex ⑤）**：webhook 到来时用 payload 的 `(github_host, installation.id)` → `github_installations` 反查 `tenant_id`（全局唯一键，见 §6）——**恰好一行**才继续，多/零行 fail-closed 拒绝；再取该租户 `github_app_credentials` 的 webhook secret 验签。凭据轮换：upsert 覆盖（secret 不留版本史，同 BYOK）。
...
**分层 + 读写 port 结构隔离（更正 Codex ⑧——靠结构非靠纪律）**：
```
GitHubAuthManager        App JWT + installation token 缓存/刷新（读写共用）
GitHubReadPort           listRepos/tree/files/issues/pulls/commits（只读）
GitHubWritePort          createIssueComment/createReview（只写；对外副作用）
  └─（复用）ssrf-guard + FieldEncryption + github_app_credentials store
```
- `GitHubReadPort` / `GitHubWritePort` 是**两个独立接口**（非同一 connector 的读写方法）。
- **WritePort 只在审批 executor 的组合根注入**（`WorkerExecutionService`/`ToolInvocationPipeline` 的 wiring）；学习路径、webhook 处理器、companion 路由**只拿到 ReadPort**，编译期就没有 WritePort 引用 → 结构上无法绕过审批直发。
- 加**架构依赖测试**：断言 WritePort 的 import 只出现在审批 executor 组合根（防未来接线漂移把 WritePort 泄漏给非 executor 模块）。

**可验证性**：真 public repo + 测试 App installation，`GitHubReadPort.listIssues(repo)` 真拉到数据 = 接通证明。token 刷新用「强制缓存过期→下次调用应静默重签」测。
...
④ GitHubResponseComposer 零-LLM 起草回应 → 存草稿，status 停在 delegated
    → 执行时 tool-risk-deriver 从写工具 metadata.highRisk 置 toolRisk='high'
    →【assessExecutionRisk 因 high 强制 requiresHuman=true，不可降级】
⑤【人工审批门】用户看到草稿 → 批准 / 驳回
    → 批准后 isExecutionApprovalCleared（绑定本次执行）
⑥ WorkerExecutionService.execute() → 写工具（highRisk，持 GitHubWritePort）→ 写（POST comment / review）
...
**GitHub 写工具（新建，注册进 ToolRegistry）**：`github-comment-tool` / `github-review-tool`，`ToolAdapter.metadata.highRisk=true`（参照 email/calendar tool），`isHighRisk(args)` 恒 true。**这两个工具是 `GitHubWritePort` 的唯一持有者**，且只经 `ToolInvocationPipeline`/`WorkerExecutionService` 调用（约束 5、⑧ 的读写 port 隔离在此落地：WritePort → 只这两个 tool → 只 executor 调）。
...
- webhook 处理器/学习/companion **只拿 GitHubReadPort**，编译期无 GitHubWritePort 引用（约束 5、⑧；架构依赖测试锁死）。
...
| github_host | TEXT | `github.com`（或 GHE host），与 app_id 一起构成全局命名空间 |
| account | TEXT | org/user 登录名 |
| repos | TEXT | 覆盖的 repo 列表（JSON）或 `*`（全部） |
| created_at / updated_at | | |

**唯一约束（更正 Codex ⑤）**：`installation_id` 在一个 GitHub host 上全局唯一，故反查键须是 **`(github_host, installation_id)` 全局唯一**（不是 `(tenant_id, installation_id)`——那不保证单 installation_id 只命中一个租户，webhook 反查会跨租户歧义）。webhook 入站用 `(github_host, installation.id)` 反查 → **恰好一行**才继续；**多行或零行 fail-closed 拒绝**（防租户混淆 / 未知 installation）。
...
| status | TEXT | `claimed` \| `ingested`（原子 claim 状态机） |
| claimed_at / ingested_at | INTEGER | |

**唯一约束**：`(tenant_id, persona_id, repo, resource_type, content_sha)`。

**原子摄入协议（更正 Codex ⑦b——check-then-act 非并发/崩溃安全）**：不用「先查后写」。改用**原子 claim + 同事务落地**：
1. `INSERT ... ON CONFLICT DO NOTHING`（status=`claimed`）——插入成功=本 worker 抢到该 content_sha 的摄入权；插入 0 行=已被抢/已摄入 → 跳过（并发安全，DB 唯一约束仲裁）。
2. 抢到后，**在同一事务内** `memoryGraph.addMemory` + 把该 digest 行 `status` 更新为 `ingested`。事务提交=记忆与账本原子一致；中途崩溃=事务回滚，claim 与记忆一起没写，下次重来（崩溃窗安全）。
3. `claimed` 但长期未 `ingested`（worker 崩在事务外的极端情况）由重扫按超时回收（reclaim）。
...
- **Plan 1（接）**：新 `github_app_credentials`（fail-closed）/`github_installations`（(github_host,installation_id) 全局唯一）表 + AuthManager + GitHubReadPort（读端点）。验证：真拉 public repo；token 强制过期→静默重签；encryption disabled→拒写凭据。
...
- **Plan 4（反馈发布接线）**：`GitHubWritePort` + github 写工具（highRisk，唯一持 WritePort，注册 ToolRegistry）+ 只在审批 executor 组合根注入。验证：审批门变异测试、绑定审批测试、**架构依赖测试**（WritePort import 只在 executor 组合根）、批准后真发到测试 repo（隔离环境）。
