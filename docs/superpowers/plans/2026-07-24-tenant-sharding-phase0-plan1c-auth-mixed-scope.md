...
>
> **第 6 轮修订**：采纳 Codex 复审 76/100 退回的 3 项——① **fingerprint 用随机盐 argon2 是真 bug**：`sha256(email+argon2hash)` 每次 argon2 随机盐→原客户端自己重试都 fingerprint 不匹配失败；裸 `sha256(email+明文)` 又可离线爆破。**改：reservation 持久化首次 argon2 hash（列名 `pending_password_hash`），续做用 `argon2.verify(pending_password_hash, 输入密码)`，shard user 复用同一 hash**（同密码稳定验证、抗爆破仍是 argon2、reservation 与 users.password_hash 一致）。② **changeEmail login 语义矛盾**：不能既「按 userId 取 user 后校验 user.email===输入」又「旧 email 窗口仍能登录（此时 shard email 已 new）」。**改：ACTIVE 目录 email 是允许登录的别名**——login 用 email→entry→userId 定位、按 userId 取 shard user、校验 `user.tenant_id===entry.tenantId`+密码，**绝不比较 shard 当前 email 与输入**。③ **v124 回填缺 operation_kind 赋值**（NOT NULL 无值→迁移崩）：回填逐类明确 operation_kind（REGISTER/TOKEN/API_KEY）、pending_password_hash=NULL、previous_lookup_value=NULL。④ **SSO/SCIM 无密码**：不能沿用 register 的密码续做双匹配声明——SSO/SCIM 用独立 reservation 路径，续做 proof 由 IdP subject/state 稳定请求身份派生（非传 NULL 冒充安全）。⑤ 统一残留签名（listPending 返回类型补 operationKind/previousLookupValue、删 deriveOperationId、resolveByEmail 字段、Task 8 authQueryUserById）。
>
...
**register 序列（有密码）**：`resolveByEmail ACTIVE→409；opId 来自客户端 Idempotency-Key（缺则一次性随机）；首次 argon2.hash(密码)→reserveTenant 存 pending_password_hash+读回 canonical (tenantId,userId,pendingHash)+reservedByUs；续做用 argon2.verify(pendingHash,密码) 证明持有原密码；若 reservedByUs 且 shard 未 COMPLETE：先事务外 Stripe(idempotencyKey=opId) 得 customerId → 再开短同步 shard 事务写租户数据(含 customerId、password_hash=pendingHash)+bootstrap(tenant_id,operation_id) COMPLETE → CAS PENDING→ACTIVE → 仅本次 reservation ACTIVE 才签发 token`。
**passwordless 序列（SSO/SCIM，无密码）**：`resolveByEmail ACTIVE→findOrCreate 既有路径 / 无则 reservePasswordlessTenant(内部随机 opId、无 Stripe、无密码)→shard 单事务写+bootstrap COMPLETE→CAS`。SSO callback state 一次性无跨请求续做，崩溃 PENDING 由恢复 worker 收敛。
login/refresh/OIDC/api-key 只认 `ACTIVE` 目录项定位 tenant，再到 shard 权威验证（login 按 entry.userId 取 shard user，email 作别名）。恢复 worker 凭匹配 operationId 的 bootstrap COMPLETE 补 ACTIVE、绝不取消 PENDING。**新增回填迁移**把升级前 users/api_keys/refresh_tokens 写成 ACTIVE 目录项（email 归一化），老用户无缝。单库下 `coordinatorDb() ≡ dbForTenant`，行为等价现状。
...
- **状态机 spec 定死（§4.1）**：PENDING→ACTIVE、不 2PC、email 唯一挡并发、只认 ACTIVE、**恢复 worker 绝不自动取消 PENDING**。客户端幂等键（register）+ `pending_password_hash` argon2.verify 续做所有权、per-op bootstrap 完成标记、CAS 失败不发 token 是落地状态机的实现约束。SSO/SCIM 无密码无跨请求续做（一次性 state），随机 operationId + bootstrap 收敛。
...
- Produces:
  ...
  - 回填 SQL（幂等）：`users.email → (tenant_id, 'email')`（**email 经 `LOWER(TRIM(email))` 归一化写入 lookup_value**）、`api_keys.key_hash → (tenant_id, 'api_key_hash')`、`refresh_tokens JOIN users` → `(tenant_id, 'refresh_token_hash')`，全部 `INSERT INTO tenant_identity_directory (tenant_id, user_id, operation_id, operation_kind, lookup_kind, lookup_value, status, created_at, updated_at) VALUES (...) ON CONFLICT(lookup_kind,lookup_value) DO NOTHING`。**逐类显式填 operation_kind + NOT NULL 列**（Codex 第 5 轮 #3：operation_kind NOT NULL 无值会崩迁移）：email 项 `operation_kind='REGISTER'`、user_id=users.id；token 项 `operation_kind='TOKEN'`、user_id=refresh_tokens.user_id；key 项 `operation_kind='API_KEY'`、user_id=NULL；三类 operation_id='backfill'、status='ACTIVE'、pending_password_hash/previous_lookup_value=NULL（老 ACTIVE 项不参与 PENDING 续做，NULL 合理）。
...
- Produces（照抄 `api-key-queries.ts`/`api-key-executors.ts` 风格）：
  - `dirCmdReserve({ tenantId, userId, operationId, operationKind, previousLookupValue, pendingPasswordHash, lookupKind, lookupValue, status, now })` → `INSERT INTO tenant_identity_directory (tenant_id, user_id, operation_id, operation_kind, previous_lookup_value, pending_password_hash, ...) VALUES (...) ON CONFLICT(lookup_kind, lookup_value) DO NOTHING`（reserve 用 status='PENDING'；record token/key 用 'ACTIVE' operation_kind='TOKEN'/'API_KEY'，userId/pendingPasswordHash 可 NULL）。
  ...
  - `dirQueryByLookup(lookupKind, lookupValue)` → `SELECT tenant_id, user_id, status, operation_id, operation_kind, previous_lookup_value, pending_password_hash FROM tenant_identity_directory WHERE lookup_kind=? AND lookup_value=? LIMIT 1`（**返全元数据**，供 reserve 读回 canonical 身份 + 续做校验 + worker 分支）。
...
**Interfaces:**
- Consumes: Task 3 executor、`TenantDbResolver.coordinatorDb()`。
- Produces（`TenantIdentityDirectory`，ctor `(private readonly resolver: TenantDbResolver)`）：
  - `reserveTenant({ tenantId, userId, operationId, pendingPasswordHash, email }): { reservedByUs: boolean; canonicalTenantId: string; canonicalUserId: string; pendingPasswordHash: string }` — register 用。**operationId 由调用方传入**（客户端 Idempotency-Key 或一次性随机）；`dirCmdReserve` PENDING（`operation_kind='REGISTER'`，带传入首次 argon2 `pendingPasswordHash`，ON CONFLICT DO NOTHING）后**读回既存行**（`dirQueryByLookup('email', canonicalizeEmail(email))`）；`reservedByUs = 读回行.operation_id === operationId`（第 6 轮：续做的密码证明改由调用方对读回行 `pending_password_hash` 做 `argon2.verify`，见 Task 5——门面只按 opId 判「是否本 reservation」，密码校验在调用方）；返回读回行的 `canonicalTenantId/canonicalUserId/pendingPasswordHash`（调用方用 canonical 身份写 shard、复用 pendingPasswordHash 作 users.password_hash）。`reservedByUs:false` → 调用方按读回行 status 转 `AUTH_EMAIL_EXISTS`(ACTIVE)/`AUTH_REGISTRATION_IN_PROGRESS`(PENDING)。
  - `reservePasswordlessTenant({ tenantId, userId, email }): { reservedByUs: boolean; operationId: string; canonicalTenantId; canonicalUserId }` — SSO/SCIM 用（无密码，第 6 轮简化）。内部**生成一次性随机 operationId**（`'reg:'+randomUUID()`）；`dirCmdReserve` PENDING（`operation_kind='REGISTER'`，pendingPasswordHash=NULL，ON CONFLICT DO NOTHING）后读回；`reservedByUs = 本次 INSERT 成功`（第 7 轮定死：读回行 operation_id===本次随机 operationId 才 true——即只有我们刚插的才属己；已有 PENDING/ACTIVE 一律 false，无「tenant_id 相等即属己」的无证明续做）。返回读回行 canonical `operationId`（调用方用它 bootstrap/activate）。`reservedByUs:false` → 调用方按读回行 status：ACTIVE→走 findOrCreate 既有用户路径、PENDING→`AUTH_REGISTRATION_IN_PROGRESS`（SSO/SCIM 无跨请求续做：state/callback 一次性，重试是新流程；崩溃-未提交 PENDING 由 Task 8 recovery 凭 bootstrap COMPLETE 收敛）。**不引入 resume_proof**（Codex 判定：一次性 state 无法续做，过度设计）。
  - `activateTenant({ email, operationId }): boolean` — CAS，rowsAffected===1。
  - `resolveByEmail(email): { tenantId; userId; status; operationKind; previousLookupValue; pendingPasswordHash } | null` — 内部 canonicalizeEmail；返全元数据供续做校验/worker 分支。
  ...
  - `listPending(cutoff): Array<{ tenantId; userId; operationId; operationKind; previousLookupValue; lookupKind; lookupValue }>` — `dirQueryPendingBefore`（恢复 worker 用，Task 8；返 operationKind/previousLookupValue 供分支）。

- [ ] **Step 1: 门面单测（红）**

`FakeMultiShardResolver`（coordinator 独立 db）。断言：`reserveTenant` 写落 **coordinator db**（非 shard），存 `pending_password_hash`；**首次 reserve 返 `reservedByUs:true` 且 canonicalTenantId/canonicalUserId===传入值 + 读回 pendingPasswordHash===传入值**；**同 operationId 重试 reserve（传不同随机 tenantId/userId）→ 仍 `reservedByUs:true` 且 canonicalTenantId===第一次的（非本次随机值）+ pendingPasswordHash===第一次的**（canonical 身份 + 密码 hash 复用的核心断言，Codex #1）；模拟他人先占（另一 operationId 手插 email 行）后本次 reserve（不同 opId）→ `reservedByUs:false`；`resolveByEmail` PENDING 时返 status=PENDING + userId + pendingPasswordHash；`reservePasswordlessTenant`（SSO/SCIM）：新 email→PENDING reservedByUs:true、email 已属他 tenant→false；activate 后 ACTIVE；`recordActiveLookup` 遇既存他租户映射 → throw；`reserveEmailChange`→新 email PENDING(operation_kind=EMAIL_CHANGE,previous=old)+旧仍 ACTIVE，`completeEmailChange`→新 ACTIVE+旧删，`rollbackEmailChange`→按 opId 删新 PENDING、旧仍 ACTIVE。

- [ ] **Step 2: 跑测试确认失败** — Expected FAIL。

- [ ] **Step 3: 写 `TenantIdentityDirectory`** — 每方法 `this.resolver.coordinatorDb()` + Task 3 executor。`reserveTenant`/`reservePasswordlessTenant` INSERT-on-conflict 后读回既存行判 reservedByUs（register 按 operationId 匹配 + 调用方 argon2.verify 密码；passwordless 按**本次随机 operationId** 匹配——冲突一律不续做，已有 PENDING/ACTIVE 均 false）。register 的 operationId 由调用方传入（Idempotency-Key/随机）；passwordless 内部随机。门面不派生 operationId。
...
6b. **⚠️PENDING 续做密码所有权（第 6 轮 Codex 修正）**：首次 register(`e`, pw1, key=`K`) 中途崩（PENDING、shard 未 COMPLETE，reservation 存首次 argon2 hash）；攻击者拿泄露的 `K` 用**不同密码** pw2 register(`e`, pw2, key=`K`) → reservedByUs:true（同 opId）但 `argon2.verify(pending_password_hash, pw2)` **失败** → 抛 `AUTH_REGISTRATION_IN_PROGRESS`、不续做、不签 token。**原客户端同 `K` 同 pw1 重试 → `argon2.verify` 成功 → 续做、shard user 复用 pending_password_hash**（关键锚：验证器用 argon2.verify 而非重算 hash 比对——同密码稳定通过，不受随机盐影响，Codex 第 5 轮真 bug 修正）。
...
const firstHash = await argon2.hash(password);      // 首次 argon2 hash；reservation 持久化它、shard user 复用它
...
const { reservedByUs, canonicalTenantId, canonicalUserId, pendingPasswordHash } =
  directory.reserveTenant({ tenantId, userId, operationId, pendingPasswordHash: firstHash, email: canonEmail });
if (!reservedByUs) {                       // opId 不属本请求（他人占/无匹配）→ 不签 token
  const cur = directory.resolveByEmail(canonEmail);
  throw cur?.status === 'ACTIVE' ? AUTH_EMAIL_EXISTS : AUTH_REGISTRATION_IN_PROGRESS;
}
// 续做密码所有权（第 6 轮）：读回行是既存 reservation（重试），须证明持有原密码
if (!(await argon2.verify(pendingPasswordHash, password))) throw AUTH_REGISTRATION_IN_PROGRESS;  // 异密码→拒，不签 token
const shardPasswordHash = pendingPasswordHash;   // shard user 复用 reservation 的 hash（同一值，稳定）
...
if (boot?.status !== 'COMPLETE') {
  ...
  resolver.dbForTenant(canonicalTenantId).transaction(() => {        // 短同步事务
    建 user(canonicalUserId,email=canonEmail,password_hash=shardPasswordHash)/subscription(customerId)/quota/identity(new IdentityWriter(canonicalTenantId, tx));
...
if (!activated) {
  ...
  // reservedByUs + argon2.verify 前置门已证本请求属本次 reservation，canonicalTenantId 来自读回行，故 tenantId+ACTIVE 两元即可
...
const entry = directory.resolveByEmail(canon);
if (!entry || entry.status !== 'ACTIVE') throw INVALID_CREDENTIALS;
const tx = resolver.dbForTenant(entry.tenantId);
const user = tx.queryOne(authQueryUserById(entry.userId));
if (!user || user.id !== entry.userId || user.tenant_id !== entry.tenantId) throw INVALID_CREDENTIALS;  // 纵深防御：目录/shard 不一致即拒
if (!(await argon2.verify(user.password_hash, password))) throw INVALID_CREDENTIALS;
generateTokenPair(app, user.id, user.tenant_id, user.role);
```
**按 entry.userId 取 shard user（非按 email）** + 校验 user.id/tenant_id 与 entry 一致——email 只作目录定位键，使 changeEmail 崩溃窗口内旧 email alias 仍能定位到已改名 user（Codex #3.1），且 `user.tenant_id===entry.tenantId` 是防目录/shard 漂移的纵深防御（Codex 第 6 轮 #2）。回填老用户目录项 user_id=users.id 故一致。
...
- `sso-user-service.ts`：ctor `(resolver, idGen?)` + `directory`；`identityWriter(tenantId)` → `new IdentityWriter(tenantId, this.resolver.dbForTenant(tenantId))`；`:47` 全局 email 查 → `directory.resolveByEmail(canonicalizeEmail(email))`；`:50` 一致性对 `entry.tenantId`；`findOrCreateForSso` 自生成走 **`reservePasswordlessTenant`**（无密码、内部随机 operationId、无 resume 续做——SSO callback state 一次性无跨请求重试语义；崩溃 PENDING 由 Task 8 recovery 凭 bootstrap COMPLETE 收敛）。route→service 无需新增 state/subject 参（reserve 的 operationId 内部生成，email 已是现有入参）。
- `scim-provisioning-service.ts`：ctor `(resolver, ...)`；`:117` 全局 email 查 → `directory.resolveByEmail(canonicalizeEmail(email))`；写走 `dbForTenant(tenantId)` 单事务 + `bootCmdMarkComplete(tenantId, operationId)`（per-op，operationId 来自 reserve 返回）。SCIM createUser tenantId 目标租户给（非自生成），email 全新时用 **`reservePasswordlessTenant`**（同 SSO，无 externalId/resume 依赖——SCIM 幂等由客户端 If-Match/既有 SCIM 语义处理，本 reserve 只保证 email 全局唯一定位；externalId 透传非本 Plan 必需，删除 resumeProof 依赖）。
...
**Interfaces:**
- Consumes: `TenantIdentityDirectory.listPending/activateTenant/completeEmailChange/rollbackEmailChange`、`TenantDbResolver`、`bootQueryByOperation`、`authQueryUserById`（读 shard canonical email，按 userId——现有工厂 auth-queries.ts:107）。
...
```
**login 崩溃窗口不锁死（Codex #3.1，第 6 轮定死语义）**：步骤 3 后步骤 4 前，旧 email 目录仍 ACTIVE 但 shard.email 已== new。**ACTIVE 目录 email 项是「允许登录的别名」**——login 语义定死为：`email→resolveByEmail(ACTIVE) entry→dbForTenant(entry.tenantId)→authQueryUserById(entry.userId)→校验 user.id===entry.userId && user.tenant_id===entry.tenantId && argon2.verify(user.password_hash, password)`。**绝不比较 shard 当前 user.email 与登录输入**（email 仅作目录定位键）。这样窗口内 login(old) 经旧 ACTIVE alias 定位到 userId → shard 按 userId 取到（email 已 new）user → 密码对即通；login(new) 因新 email 还 PENDING 定位不到 → 暂拒（恢复后通）——**旧 email 始终可登录**，无「两 email 都登不上」。
...
- **Codex 第 5 轮 2 实质 + 3 记录**：① PENDING 续做所有权（第 6 轮改为 `pending_password_hash`+argon2.verify，见下）；② changeEmail 恢复——目录加 `operation_kind`+`previous_lookup_value`，Task 8 worker 加 EMAIL_CHANGE 分支，login 按 userId ✅；③ PENDING-squat DoS 记录 ✅；④ `AUTH_REGISTRATION_IN_PROGRESS` HTTP 409+contract ✅；⑤ reserve 由调用方传 operationId ✅。
- **Codex 第 6 轮 4 项定稿**：① fingerprint 随机盐 bug——改 reservation 持久化首次 argon2 hash(`pending_password_hash`)、续做 `argon2.verify(pending_password_hash, 输入密码)`（非重算 hash 比对，同密码稳定跨随机盐）、shard user 复用同 hash ✅（Task 1/3/4/5.3+5.1.6b；argon2.verify 是现有 `@node-rs/argon2` API，login 已用）；② changeEmail login 别名语义定死——**删 Task 9 的「比较 shard user.email 与输入」矛盾句** + Task 5 login 显式 `user.id===entry.userId && user.tenant_id===entry.tenantId && argon2.verify` ✅（Task 5 login + Task 9）；③ v124 回填逐类 operation_kind+全 NOT NULL 列 ✅（Task 2）；④ **SSO/SCIM resume 过度设计删除**——`reserveSsoTenant`+resumeProof 替换为 `reservePasswordlessTenant`（随机 operationId、无 resume、无 externalId 依赖；SSO state 一次性无跨请求续做、崩溃 PENDING 由 bootstrap 收敛）✅（Task 4/6，删 resume_proof 列）；⑤ 残留签名统一（listPending/resolveByEmail/authQueryUserById/删 deriveOperationId/清 request_fingerprint 陈述）✅。
- **Placeholder 扫描**：DSL builder API 标「以 v078/v108 为准」（防自创非 placeholder）；legacy fixture「照末条 raw 风格加对应版本」是确定动作非 TBD。
- **类型一致**：`reserveTenant` 返 `{reservedByUs, canonicalTenantId, canonicalUserId, pendingPasswordHash}`（operationId 由调用方传入）、`reservePasswordlessTenant` 返 `{reservedByUs, operationId, canonicalTenantId, canonicalUserId}`（内部随机 operationId，无 resume；返回 canonical operationId 供 bootstrap/activate）、`activateTenant` 返 boolean、`resolveByEmail` 返 `{tenantId, userId, status, operationKind, previousLookupValue, pendingPasswordHash}|null`、`resolveBy*Hash` 返 `{tenantId}|null`、`recordActiveLookup`/`removeLookup`/`reserveEmailChange`/`completeEmailChange`/`rollbackEmailChange`/`listPending` 全 Task 引用一致；`dirCmdReserve` 带 userId+operationId 入参、`bootCmdMarkComplete`/`bootQueryByOperation`（per-op）一致；`idGen: {tenantId();userId()}` seam 一致；`register(email, password, opts?:{idempotencyKey?})`；`createCustomer(config,email,tenantId,idempotencyKey?)`；`canonicalizeEmail` 单一真源；错误码 `AUTH_REGISTRATION_RETRY` + `AUTH_REGISTRATION_IN_PROGRESS` 在 Task 5 注册。
