...
import type { KernelClock, KernelLogger } from '../../ports/host-adapters.js';
import type { CoreValue } from './value-types.js';
...
export const DEFAULT_PATTERN_EXTRACTION_CONFIG: PatternExtractionConfig = {
  minMemoryCount: 5,
...
export function extractPatterns(
  ...
  for (const m of memories.values()) {
    if (m.kind === 'semantic') semanticMemories.push(m);
  }
  ...
  for (const value of values.values()) {
    ...
    for (const mem of semanticMemories) {
      if (hasTokenOverlap(valueTokens, tokenize(mem.content))) related.push(mem);
    }
    if (related.length < config.minMemoryCount) continue;
    ...
    if (Math.abs(valenceAvg) < config.valenceThreshold) continue;

    const rawDelta = valenceAvg * salienceAvg * config.driftSensitivity;
    const suggestedWeightDelta = Math.max(-config.maxDriftDelta, Math.min(config.maxDriftDelta, rawDelta));
    ...
    logger?.info(`模式提取: ${value.label} — ${related.length} 条记忆, 建议漂移 ${suggestedWeightDelta.toFixed(4)}`);
  }
  return patterns;
}
 ...
 * 将模式转换为价值更新提案
 */
export function patternsToProposals(
 ...
 * 与关联价值维度产生即时漂移提案
 */
export function extractEmotionalEvents(
  config: PatternExtractionConfig,
  memories: ReadonlyMap<string, MemoryNode>,
  values: ReadonlyMap<string, CoreValue>,
  logger?: KernelLogger,
  ...
  for (const mem of memories.values()) {
    ...
    if (intensity < config.emotionalEventThreshold) continue;
    ...
    for (const value of values.values()) {
      ...
      const rawDelta = mem.valence * mem.salience * config.emotionalDriftSensitivity;
      const clampedDelta = Math.max(-config.maxDriftDelta, Math.min(config.maxDriftDelta, rawDelta));
      ...
      logger?.info(`情绪事件触发: ${value.label} — 记忆 ${mem.id}, 建议漂移 ${delta.toFixed(4)}`);
    }
  }
  return proposals;
}
