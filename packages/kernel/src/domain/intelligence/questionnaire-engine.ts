...
/**
 * 评估用户答案，推断 L2/L3 参数（纯函数）
 */
export function evaluateQuestionnaire(responses: readonly QuestionResponse[]): InferredParameters {
  const responseMap = new Map(responses.map(r => [r.id, r.score]));

  const dimensionScores = new Map<string, { total: number; count: number }>();

  for (const q of QUESTIONS) {
    const rawScore = responseMap.get(q.id);
    if (!Number.isFinite(rawScore)) continue;
    const score = Math.max(1, Math.min(5, rawScore as number));
