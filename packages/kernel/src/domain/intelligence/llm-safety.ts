/**
 * LLM 安全管道 — 纯领域逻辑
 * 提示注入检测 + 输出验证（纯函数，无副作用）
 * 零 node:* 依赖
 */

import type { ChatMessage, ChatResponse } from './llm-provider-types.js';
...
export interface SafetyMetrics {
  ...
  inputBlocked: number;
  outputChecks: number;
  outputSanitized: number;
}

/** 创建零值安全指标 */
export function createSafetyMetrics(): SafetyMetrics {
  return { inputChecks: 0, inputBlocked: 0, outputChecks: 0, outputSanitized: 0 };
}
 ...
 */
const INJECTION_PATTERNS: readonly { readonly pattern: RegExp; readonly severity: 'medium' | 'high' | 'critical'; readonly description: string }[] = [
  {
...
/** 输出中不应包含的敏感信息模式 */
const OUTPUT_SENSITIVE_PATTERNS: readonly { readonly pattern: RegExp; readonly description: string }[] = [
...
/** 检查输入消息是否包含提示注入（纯函数） */
export function checkInputSafety(messages: readonly ChatMessage[], metrics?: SafetyMetrics): SafetyCheckResult {
  if (metrics) metrics.inputChecks++;
  ...
  for (const msg of messages) {
    ...
    for (const { pattern, severity, description } of INJECTION_PATTERNS) {
      if (pattern.test(msg.content)) {
        if (metrics) metrics.inputBlocked++;
        return {
          safe: false,
...
/** 验证和清理 LLM 输出（纯函数） */
export function validateOutput(response: ChatResponse, metrics?: SafetyMetrics): ChatResponse {
  if (metrics) metrics.outputChecks++;
